...
/**
 * @author ljjy1
 * @classname ExpandRequest
 * @date 2023/10/12 15:29
 */
@Data
public class ExpandRequest extends BaseRequest implements Serializable {
    ...
    public void check(){
        ...
        if(StrUtil.isBlank(direction) || !Arrays.asList("right","left","up","down").contains(direction.toLowerCase())){
            throw new DiscordMjJavaException("缩小比例不能为空,且只能选择50,75 [The direction cannot be empty and can only be right or left or up or down]");
        }
    }
}
